...
void wrq_recv_callback(void *arg, struct udp_pcb *upcb, struct pbuf *pkt_buf, struct ip_addr *addr, u16_t port)
{
  ...
  {
#ifdef USE_FILE_SYSTEM_IN_TFTP
    f_write(&file_CR, (char*)pkt_buf->payload + TFTP_DATA_PKT_HDR_LEN, pkt_buf->len - TFTP_DATA_PKT_HDR_LEN, (UINT*)&n);
#else /* USE_FILE_SYSTEM_IN_TFTP */
    /* instead of f_write, we write data to flash directly.*/
//    FLASH_Unlock();
    if (FlashProgram((uint32_t*)((char*)pkt_buf->payload + TFTP_DATA_PKT_HDR_LEN), g_ui32WritingPosition, pkt_buf->len - TFTP_DATA_PKT_HDR_LEN) != 0) {
    	/*
    	 * a flash writing error happens here.
    	 * report this error
    	 * */
    	g_ui8DownloadError = 1;
    }
    else {
    	g_ui32WritingPosition += (pkt_buf->len - TFTP_DATA_PKT_HDR_LEN);
    	g_ui32DownloadedSize += (pkt_buf->len - TFTP_DATA_PKT_HDR_LEN);
    	n = g_ui32DownloadedSize;
    }
//    FLASH_lock();
#endif /* USE_FILE_SYSTEM_IN_TFTP */
...
#else
  /*
   * instead of open file here, we do nothing related to file.
   * flash_erase is done in previous step.
   * we just init the pointer of writing operation here to download_buf_addr
   * */
  g_ui32WritingPosition = g_ui32DownloadBuffAddr;
  g_ui32DownloadedSize = 0;
#endif
...
#else
      /*
       * This is a tftp writing req from the client.
       * Instead of openning the file, we erase the download buffer.
       * */
      g_ui8IsWrqReceived = 1;
      uint32_t i = 0;
      for (i=0; i<256; i++) {
    	  if (FlashErase(g_ui32DownloadBuffAddr + i * 1024) != 0) {
    		  // report the erase error.
    		  g_ui8FlashError = 1;
    	  }
      }
#endif
